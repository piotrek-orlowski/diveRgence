...
spotVol <-  function(rdata, spot.index = NULL, makeReturns, align.by, align.period, apply.at, apply.period, estFun = spotVolBaseContinuous, avg.vol, intradaySeasonFun, reference.time= "07:30:00", ...){
  ...
  # Apply estimator
  result <- sapply(X = spot.index, FUN = estFun, rdata = rdata, align.by = align.by, align.period = align.period, makeReturns = makeReturns, avg.vol = avg.vol, intradaySeasonFun = intradaySeasonFun, reference.time = reference.time, ...)
...
spotVolBaseJump <- function(spot.point, rdata, align.by, align.period, makeReturns, avg.vol, intradaySeasonFun, reference.time, ...){
  ...
  T.range <- as.numeric(difftime(T.range[2],T.range[1], units = "secs"))
  delta.bar <- T.range / (length(rdata)-1)
  delta.bar.years <- (delta.bar/(86400*365)) # in business time
  
  # Vol seasonality
  reference.time <- as.POSIXlt(reference.time, format = "%H:%M:%S")
  reference.time$year <- as.POSIXlt(spot.point[1])$year
  reference.time$mon <- as.POSIXlt(spot.point[1])$mon
  reference.time$mday <- as.POSIXlt(spot.point[1])$mday
  # time.stamp <- sapply(index(rdata), function(x) as.numeric(difftime(time1 = x, time2 = reference.time, units = "sec")))
  time.stamp <- as.numeric(index(rdata) - reference.time)
  
  # Find a kernel
  delta.bandwidth <- exp(1)*length(rdata)^0.35 * delta.bar
  
  # Calculate estimator
  time.kernel <- expDoubleKernel(x0 = spot.point, cnv.data = index(rdata), delta.bandwidth = delta.bandwidth)
  time.kernel.minus <- time.kernel * as.numeric(index(rdata) < spot.point)
